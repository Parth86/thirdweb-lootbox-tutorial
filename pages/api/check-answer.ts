...
//We initialize a transfer by calling packModule.transfer(address, packTokenId, BigNumber.from(1)); and then we return a correct response to the user. Note that this works because the account associated with the WALLET_PRIVATE_KEY owns all instances of the packs, so it can transfer one to the user.
